(function(){
  ...
  const SURFACE_ART={mossveinGround:null,mossveinMinePath:null,roads:{},moonglassGround:null,moonglassGroundPattern:null,moonglassCrystals:null,moonglassBloomBed:null,emberdeepGround:null,emberdeepGroundPattern:null,emberdeepSlag:null,emberdeepFaultBed:null,emberdeepMinePath:null};
  ...
  const STARTER_ART={sellStation:null,forgeStation:null,storageChest:null,wayfarerShop:null,moonglassGate:null,moonglassGateMark:null,emberdeepSeal:null,emberdeepSealMark:null,treasureClosed:null,treasureOpen:null,drops:{}};
  const STARTER_PATHS=Object.freeze({
    ...
    treasureClosed:'assets/surface/treasure-cache-closed.png',treasureOpen:'assets/surface/treasure-cache-open.png',moonglassGate:'assets/surface/moonglass-gate.png',moonglassGateMark:'assets/surface/moonglass-gate-mark.png',emberdeepSeal:'assets/surface/emberdeep-seal.png',emberdeepSealMark:'assets/surface/emberdeep-seal-mark.png'
  });
  ...
  const MINE_ENTRANCE_PATHS=Object.freeze({mossMine:'assets/entrances/mossvein-entrance.png',moonMine:'assets/entrances/moonglass-entrance.png',emberMine:'assets/entrances/emberdeep-entrance.png',depthLamp:'assets/entrances/depth-work-lamp.png'});
  ...
  const BUILD={version:'0.28.3',name:'EMBERDEEP COMPLETE'};
  ...
  const CHEST_DEFINITIONS=[
    ...
    {id:'star_cache',name:'Astral Cache',biome:'starfall',x:3495,y:1110,tier:3,requires:{pickaxeLevel:5,label:'Ember Pickaxe'},rewards:{coin:800}},
  ...
  let moonglassGateTransition=null,emberdeepGateTransition=null;
  let saleMotes=[];
  ...
  function spawnDiscoveryBurst(rock,label){
    ...
    if(rare)floaters.push({x:rock.x,y:rock.y-52,text:label,color:data.edge,age:0,life:1.65,size:19});
    miningFeedback.lastDiscovery={type:rock.type,label,rare};
    if(rare)showAreaBanner(label);sound('discovery',rock.type);miningKick(rare?9:7,rare?.24:.18,data.edge,rare?30:22);
  }
  ...
  function activeMineProductionArt(){return isPrismaticProduction()?PRISMATIC_ART:isMoonglassProduction()?MOONGLASS_ART:isMoltenProduction()?MOLTEN_ART:isEmberdeepProduction()?EMBERDEEP_ART:null}
  function activeMoonProductionArt(){return activeMineProductionArt()}
  function mineVisualPass(){return isMoltenProduction()?'molten-production-assets-v1':isEmberdeepProduction()?'emberdeep-production-assets-v1':isPrismaticProduction()?'prismatic-production-assets-v1':isMoonglassProduction()?'moonglass-production-assets-v1':isRootwoundProduction()?'rootwound-production-assets-v1':isMossveinVisual(currentMineVisual())?'mossvein-production-art-v2':'legacy'}
  ...
  function drawMoonProductionFloor(mine){
    ...
    if(pattern){ctx.save();ctx.fillStyle=pattern;ctx.globalAlpha=deep?.92:.96;ctx.fillRect(0,0,mine.width,mine.height);ctx.globalAlpha=1;ctx.fillStyle=currentScene==='emberMine'?(deep?'rgba(30,5,2,.22)':'rgba(31,8,5,.1)'):(deep?'rgba(7,7,19,.24)':'rgba(5,16,20,.12)');ctx.fillRect(0,0,mine.width,mine.height);ctx.restore()}
    ...
    if(!deep&&imageReady(art.routeMarker)){
      const route=currentScene==='emberMine'?[[310,1025,0],[555,630,-.55],[820,675,.06],[1080,680,-.18],[1265,455,-.4],[1545,455,0],[1705,970,.58]]:[[350,1180,0],[535,700,-.62],[790,700,0],[1055,500,-.38],[1320,350,-.2],[1530,350,0]];
  ...
  function drawMoonProductionWallArt(terrain,concealed,visible,art){
    ...
    for(const item of placements){
      const salt=currentScene==='emberMine'?(currentDepth===2?359:337):(isPrismaticProduction()?307:281),n=visualNoise(item.col,item.row,salt),width=174+n*26,height=width*(image.naturalHeight/image.naturalWidth);let x=(item.col+.5)*MINE_TILE_SIZE-camera.x,y=(item.row+.5)*MINE_TILE_SIZE-camera.y,rotation=0;
  ...
  function drawMoonProductionTerrain(mine,terrain,startCol,endCol,startRow,endRow,target){
    ...
    if(pattern){ctx.save();ctx.translate(-camera.x,-camera.y);ctx.fillStyle=pattern;ctx.globalAlpha=currentDepth===2?.7:.76;ctx.fillRect(camera.x-64,camera.y-64,viewWidth+128,viewHeight+128);ctx.globalAlpha=1;ctx.fillStyle=currentScene==='emberMine'?(currentDepth===2?'rgba(38,8,3,.51)':'rgba(46,14,8,.47)'):(isPrismaticProduction()?'rgba(8,5,24,.57)':'rgba(4,22,29,.52)');ctx.fillRect(camera.x-64,camera.y-64,viewWidth+128,viewHeight+128);ctx.restore()}ctx.restore();
    for(const cell of visible){
  ...
  function drawMoonSolidWall(wall,p){
    const art=activeMineProductionArt(),pattern=productionFloorPattern(art),image=art&&art.wall,mine=currentMineVisual();ctx.save();ctx.translate(p.x,p.y);ctx.fillStyle=mine.floor;ctx.fillRect(0,0,wall.w,wall.h);
    if(pattern){ctx.save();ctx.globalAlpha=.64;ctx.fillStyle=pattern;ctx.fillRect(0,0,wall.w,wall.h);ctx.globalAlpha=1;ctx.fillStyle=currentScene==='emberMine'?'rgba(35,8,4,.5)':'rgba(3,15,20,.52)';ctx.fillRect(0,0,wall.w,wall.h);ctx.restore()}
    if(imageReady(image)){
  ...
  function drawMineWalls(){
    ...
    for(const wall of mine.solids){
      ...
      if(activeMineProductionArt()){drawMoonSolidWall(wall,p);continue}
  ...
  function drawDepthEntrance(){
    ...
    const production=currentScene==='mossMine'?ROOTWOUND_ART.shaft:currentScene==='moonMine'?PRISMATIC_ART.shaft:currentScene==='emberMine'?MOLTEN_ART.shaft:null;
    if(imageReady(production)){
      ...
      ctx.save();ctx.translate(p.x,p.y);ctx.fillStyle='rgba(0,0,0,.4)';ctx.beginPath();ctx.ellipse(0,35,62,19,0,0,Math.PI*2);ctx.fill();ctx.drawImage(production,-assetWidth*.5,40-assetHeight,assetWidth,assetHeight);if(currentScene!=='emberMine')drawDepthEntranceLamp(mine);
  ...
  function drawSurfaceEmberdeepGround(){
    ...
    const blendStart=WORLD.emberGateX-EMBERDEEP_SURFACE_BLEND,blendEnd=WORLD.emberGateX+EMBERDEEP_SURFACE_BLEND,steps=48,stepWidth=(blendEnd-blendStart)/steps;
    ctx.save();ctx.translate(-camera.x,-camera.y);
    if(pattern){
      ctx.fillStyle=pattern;ctx.fillRect(blendEnd,0,WORLD.starfallGateX-blendEnd,WORLD.height);
      for(let index=0;index<steps;index++){const raw=(index+.5)/steps,alpha=raw*raw*(3-2*raw);ctx.globalAlpha=alpha;ctx.fillRect(blendStart+index*stepWidth,0,stepWidth+1,WORLD.height)}
    }else ctx.drawImage(image,WORLD.emberGateX,0,WORLD.starfallGateX-WORLD.emberGateX,WORLD.height);
    ctx.globalAlpha=1;ctx.fillStyle='rgba(54,11,5,.13)';ctx.fillRect(blendEnd,0,WORLD.starfallGateX-blendEnd,WORLD.height);
  ...
  function drawGround(){
    ...
    const cavernStart=worldToScreen(WORLD.gateX,0).x,emberStart=worldToScreen(WORLD.emberGateX,0).x,starfallStart=worldToScreen(WORLD.starfallGateX,0).x,moonProduction=imageReady(SURFACE_ART.moonglassGround),emberProduction=imageReady(SURFACE_ART.emberdeepGround);
    ...
    const legacyGridStart=emberProduction?starfallStart:moonProduction?emberStart:cavernStart;ctx.save();ctx.beginPath();ctx.rect(legacyGridStart,0,viewWidth-legacyGridStart,viewHeight);ctx.clip();ctx.translate(-camera.x%80,-camera.y%80);ctx.strokeStyle='rgba(190,205,165,.045)';ctx.lineWidth=1;
  ...
  function drawDepthStations(){
    const stations=depthStations(),mine=currentMineVisual(),production=currentScene==='mossMine'?ROOTWOUND_ART:currentScene==='moonMine'?PRISMATIC_ART:currentScene==='emberMine'?MOLTEN_ART:null;
  ...
  function drawEmberdeepGateMark(progress){
    const image=STARTER_ART.emberdeepSealMark,p=worldToScreen(WORLD.emberGateX,WORLD.gateY);if(!imageReady(image)||p.x<-140||p.x>viewWidth+140)return;
    const eased=progress*progress*(3-2*progress),scale=Math.min(192/image.naturalWidth,126/image.naturalHeight),assetWidth=image.naturalWidth*scale,assetHeight=image.naturalHeight*scale;
    ctx.save();ctx.translate(p.x,p.y);ctx.globalAlpha=.2+.8*eased;ctx.drawImage(image,-assetWidth*.5,73-assetHeight*.5,assetWidth,assetHeight);ctx.restore();
  }
  ...
  function drawMineralNodeAsset(rock){
    ...
    const activeArt=activeMineProductionArt(),productionNode=activeArt&&activeArt.nodes[rock.type]||currentScene==='surface'&&(EMBERDEEP_ART.nodes[rock.type]||MOONGLASS_ART.nodes[rock.type]);
  ...
  function drawChests(){
    for(const chest of chests){
      ...
      const biomeChest=MOONGLASS_SURFACE_CHEST_ART[chest.id]||EMBERDEEP_SURFACE_CHEST_ART[chest.id],production=biomeChest?(opened?biomeChest.open:biomeChest.closed):chest.biome==='mossvein'?(opened?STARTER_ART.treasureOpen:STARTER_ART.treasureClosed):null;
      if(imageReady(production)){
  ...
  function playerRenderPose(){
    ...
    if(drilling){
      const buzzX=activeDrill?-.2+Math.sin(time*79)*.55+Math.sin(time*43)*.15:0,buzzY=activeDrill?Math.sin(time*91)*.3:0;
      return{toolAngle:0,armAngle:0,armX:buzzX,armY:buzzY,bodyAngle:activeDrill?Math.sin(time*61)*.004:0,bodyY:0,active:activeDrill};
    }
