(function(){
  ...
  const BUILD={version:'0.37.3',name:'GATE AWAKENING'};
  const PATCH_NOTES=[
  ...
  function drawGateAt(x,open,glowColor,stoneColor,braceColor,production=null,transitionProgress=0){
    ...
    if(imageReady(production)){
      ctx.save();ctx.globalAlpha=.4*(1-pose.sink);ctx.fillStyle='rgba(0,0,0,.4)';ctx.beginPath();ctx.ellipse(0,101,102,24,0,0,Math.PI*2);ctx.fill();ctx.restore();
    }
    ...
    if(imageReady(production)){
      ...
      if(!open||transitioning){ctx.save();ctx.strokeStyle=glowColor;ctx.lineWidth=7;ctx.shadowBlur=20;ctx.shadowColor=glowColor;ctx.globalAlpha=!open?.92:(1-pose.sink)*(.72+.28*pose.flash);ctx.beginPath();ctx.moveTo(0,-48);ctx.lineTo(0,81);ctx.stroke();ctx.restore()}
      ctx.restore();return;
    }
