...
assert.equal(latest.version,'0291');
assert.match(html,/version\.json\?t=/);
assert.match(html,/cache:'no-store'/);
assert.match(html,/style\.css\?v=0291/);
assert.match(html,/script\.js\?v=0291/);
assert.match(html,/assets\/branding\/ever-deeper-logo\.png\?v=0291/);
assert.match(html,/assets\/characters\/miner-b-walk\.png\?v=0291/);
...
let api=runtime.api;
assert.equal(api.snapshot().build.version,'0.29.1');
assert.equal(api.snapshot().build.name,'EMBER PATH ALIGNMENT');
assert.equal(runtime.elements.get('buildVersion').textContent,'v0.29.1');
assert.equal(api.snapshot().assetVersion,'0291');
...
assert.equal(JSON.stringify(api.startMusic()),JSON.stringify({src:'assets/audio/ever-deeper-drift-loop.mp3?v=0291',volume:1,loop:true,paused:false}));
