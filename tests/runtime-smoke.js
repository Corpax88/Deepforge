...
assert.equal(latest.version,'0321');
assert.match(html,/version\.json\?t=/);
assert.match(html,/cache:'no-store'/);
assert.match(html,/style\.css\?v=0321/);
assert.match(html,/script\.js\?v=0321/);
assert.match(html,/assets\/branding\/ever-deeper-logo\.png\?v=0321/);
assert.match(html,/assets\/characters\/miner-b-walk\.png\?v=0321/);
assert.match(html,/id="toolIcon"[^>]+assets\/tools\/pickaxe-worn\.png\?v=0321/);
assert.match(html,/id="mineToolIcon"[^>]+assets\/tools\/pickaxe-worn\.png\?v=0321/);
...
let api=runtime.api;
assert.equal(api.snapshot().build.version,'0.32.1');
assert.equal(api.snapshot().build.name,'DEEPGLASS PREMIUM');
assert.equal(runtime.elements.get('buildVersion').textContent,'v0.32.1');
assert.equal(api.snapshot().assetVersion,'0321');
...
assert.equal(JSON.stringify(api.startMusic()),JSON.stringify({src:'assets/audio/ever-deeper-drift-loop.mp3?v=0321',volume:1,loop:true,paused:false}));
