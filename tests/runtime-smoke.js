...
assert.equal(latest.version,'0344');
assert.match(html,/version\.json\?t=/);
assert.match(html,/cache:'no-store'/);
assert.match(html,/style\.css\?v=0344/);
assert.match(html,/script\.js\?v=0344/);
assert.match(html,/assets\/branding\/ever-deeper-logo\.png\?v=0344/);
assert.match(html,/assets\/characters\/miner-b-walk\.png\?v=0344/);
assert.match(html,/id="toolIcon"[^>]+assets\/tools\/pickaxe-worn\.png\?v=0344/);
assert.match(html,/id="mineToolIcon"[^>]+assets\/tools\/pickaxe-worn\.png\?v=0344/);
assert.match(html,/rel="manifest" href="manifest\.webmanifest\?v=0344"/);
...
assert.match(source,/const PATCH_NOTES=\[/);assert.match(source,/version:'0\.34\.4'.+title:'Readable Cave Text'/);assert.match(css,/\.patch-note\.latest/);
...
let api=runtime.api;
assert.equal(api.snapshot().build.version,'0.34.4');
assert.equal(api.snapshot().build.name,'DEEPGLASS PREMIUM');
assert.equal(runtime.elements.get('buildVersion').textContent,'v0.34.4');
assert.equal(api.snapshot().assetVersion,'0344');
assert.equal(api.snapshot().patchNotes.length,5);assert.equal(api.snapshot().patchNotes[0].version,'0.34.4');assert.match(api.snapshot().patchNotes[0].items.join(' '),/above the lighting pass/);assert.match(api.snapshot().patchNotes[3].items.join(' '),/100 Emberstone/);assert.match(api.snapshot().patchNotes[3].items.join(' '),/200 Astralite and 200 Crownstone/);assert.equal(api.snapshot().lighting.readableTextOverlay,true);assert.equal(api.snapshot().lighting.textPass,'after-lighting');
...
assert.equal(JSON.stringify(api.startMusic()),JSON.stringify({src:'assets/audio/ever-deeper-drift-loop.mp3?v=0344',volume:1,loop:true,paused:false}));
