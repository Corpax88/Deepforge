...
let api=runtime.api;
assert.equal(api.snapshot().build.version,'0.12.0');
assert.equal(api.snapshot().build.name,'MOSSVEIN VISUAL REWORK');
assert.equal(runtime.elements.get('buildVersion').textContent,'v0.12.0');
...
assert.equal(api.snapshot().mine.visualPass,'mossvein-premium-v1');
