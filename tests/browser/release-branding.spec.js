...
test('Ever Deeper release branding is complete and mobile-safe',async({page})=>{
  ...
  const gameDataIndex=scripts.findIndex(script=>script.src==='game-data.js?v=0352'),engineIndex=scripts.findIndex(script=>script.src==='script.js?v=0352');
  expect(gameDataIndex).toBeGreaterThanOrEqual(0);expect(engineIndex).toBeGreaterThan(gameDataIndex);
  expect(scripts.every(script=>script.type===''&&script.src.endsWith('?v=0352'))).toBe(true);
  ...
  await expect(logo).toHaveAttribute('src','assets/branding/ever-deeper-logo.png?v=0352');
  ...
  await expect(page.locator('#toolIcon')).toHaveAttribute('src','assets/tools/pickaxe-worn.png?v=0352');
  await expect(page.locator('#mineToolIcon')).toHaveAttribute('src','assets/tools/pickaxe-worn.png?v=0352');
  ...
  await expect(page.locator('#contextIconImage')).toHaveAttribute('src',/assets\/surface\/forge-station\.png\?v=0352$/);
  ...
  expect(release).toEqual({
    build:{version:'0.35.2',name:'BEDROCK LANGUAGE'},
