...
test('Ever Deeper release branding is complete and mobile-safe',async({page})=>{
  ...
  await expect(logo).toHaveAttribute('src','assets/branding/ever-deeper-logo.png?v=0321');
  ...
  await expect(page.locator('#toolIcon')).toHaveAttribute('src','assets/tools/pickaxe-worn.png?v=0321');
  await expect(page.locator('#mineToolIcon')).toHaveAttribute('src','assets/tools/pickaxe-worn.png?v=0321');
  await page.evaluate(()=>window.__everDeeperTest.setPosition(455,250));
  await expect(page.locator('#contextPanel')).toBeVisible();
  await expect(page.locator('#contextIconImage')).toHaveAttribute('src',/assets\/surface\/forge-station\.png\?v=0321$/);
  ...
  expect(release).toEqual({
    build:{version:'0.32.1',name:'DEEPGLASS PREMIUM'},
