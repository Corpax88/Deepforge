...
test('Ember Mastery enforces both Sunslag and gold requirements',async({page})=>{
  ...
  await expect(page.locator('#contextDetail')).toContainText('0 / 100');

  await page.evaluate(()=>window.__everDeeperTest.grantGold(450));
  await expect(page.locator('#contextButton')).toBeDisabled();
  await page.evaluate(()=>window.__everDeeperTest.grantCargo('sunslag',100));
...
test('all five Ember Mastery ranks improve mining and persist',async({page},testInfo)=>{
  ...
  await page.evaluate(()=>{
    window.__everDeeperTest.unlockAllAreas();
    window.__everDeeperTest.setPickaxeLevel(5);
    window.__everDeeperTest.grantCargo('sunslag',500);
...
test('Ember Mastery remains readable on an iPhone viewport',async({page},testInfo)=>{
  ...
  await page.evaluate(()=>{
    window.__everDeeperTest.unlockAllAreas();
    window.__everDeeperTest.setPickaxeLevel(5);
    window.__everDeeperTest.grantCargo('sunslag',100);
...
test('every mine requires its final ore gate before the right-side deep descent',async({page})=>{
  ...
  for(const scene of ['mossMine','moonMine','emberMine','starMine']){
    ...
    expect(wall).toMatchObject({x:0,role:'deepAccessWall',barrierId:snapshot.mine.barrierIds.at(-1)});expect(snapshot.mine).toMatchObject({deepAccessPremiumAsset:true,legacyCanvasDeepAccessFace:false,visibleWallFaces:true});expect(snapshot.mine.deepAccessWallAsset).toMatch(/\.png$/);expect(wall.w).toBeLessThan(snapshot.mine.width-52);expect(await page.evaluate(({x,y})=>window.__everDeeperTest.mineCollisionAt(x,y),{x:wall.w-80,y:wall.y+70})).toBe(true);
...
test('expanded mine depths use lazy terrain chunks and a following camera',async({page})=>{
  ...
  await page.evaluate(()=>window.__everDeeperTest.enterMine('mossMine'));
  let snapshot=await page.evaluate(()=>window.__everDeeperTest.snapshot());
  expect(snapshot.build).toEqual({version:'0.35.1',name:'GATEBOUND DEPTHS'});
  expect(snapshot.assetVersion).toBe('0351');
...
test('the exact build version is always visible in the game HUD',async({page})=>{
  await freshGame(page);
  await expect(page.locator('#buildVersion')).toHaveText('v0.35.1');
  ...
  await expect(page.locator('#menuBuildVersion')).toHaveText('EVER DEEPER v0.35.1 · GATEBOUND DEPTHS');
});
...
test('premium start menu owns continue, new game, achievements, and settings',async({page})=>{
  ...
  await expect(page.locator('.start-logo')).toHaveAttribute('src','assets/branding/ever-deeper-logo.png?v=0351');
...
test('settings opens first and keeps audio choices separate from stats',async({page})=>{
  ...
  await expect(page.locator('.patch-note')).toHaveCount(10);
  await expect(page.locator('.patch-note.latest')).toContainText('v0.35.1');
  await expect(page.locator('.patch-note.latest')).toContainText('production PNGs');
  await expect(page.locator('.patch-note').nth(1)).toContainText('Physical biome walls');
  await expect(page.locator('.patch-note').nth(2)).toContainText('Gameplay, balance, progression, controls, and visuals are unchanged');
  await expect(page.locator('.patch-note').nth(3)).toContainText('Reset All Progress');
  await expect(page.locator('.patch-note').nth(8)).toContainText('100 Emberstone');
  await expect(page.locator('.patch-note').nth(8)).toContainText('200 Astralite and 200 Crownstone');
...
test('one text-free visual guide leads to the next action and fades nearby',async({page})=>{
  ...
  expect(snapshot.miningFeedbackRendering).toEqual({routineImpactRings:false,routineBreakRings:false,drillVibration:true,drillVibrationMaxOffset:2.05,cameraShake:false});
...
test('v0.35.1 exposes the complete production contracts and premium walk renderer',async({page})=>{
  await freshGame(page);
  const snapshot=await page.evaluate(()=>window.__everDeeperTest.snapshot());
  expect(snapshot.build).toEqual({version:'0.35.1',name:'GATEBOUND DEPTHS'});
  expect(snapshot.assetVersion).toBe('0351');
...
test('Emberdeep seal sinks through start, middle, and finish before leaving its permanent mark',async({page})=>{
  ...
  const stages=await page.evaluate(()=>{
    ...
    api.step(.7);api.renderOnce();const middle=api.snapshot().emberdeepGateTransition;
