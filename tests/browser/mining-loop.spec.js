...
test('the exact build version is always visible in the game HUD',async({page})=>{
  await freshGame(page);
  await expect(page.locator('#buildVersion')).toHaveText('v0.26.8');
  ...
  await expect(page.locator('#menuBuildVersion')).toHaveText('EVER DEEPER v0.26.8 · MOONGLASS COMPLETE');
});
...
test('v0.26.8 exposes the complete Moonglass and Prismatic production contracts',async({page})=>{
  await freshGame(page);
  const snapshot=await page.evaluate(()=>window.__everDeeperTest.snapshot());
  expect(snapshot.build.version).toBe('0.26.8');
  expect(snapshot.assetVersion).toBe('0268');
