...
test('expanded mine depths use lazy terrain chunks and a following camera',async({page})=>{
  ...
  await page.evaluate(()=>window.__everDeeperTest.enterMine('mossMine'));
  let snapshot=await page.evaluate(()=>window.__everDeeperTest.snapshot());
  expect(snapshot.build).toEqual({version:'0.34.5',name:'DEEPGLASS PREMIUM'});
  expect(snapshot.assetVersion).toBe('0345');
...
test('premium start menu owns continue, new game, achievements, and settings',async({page})=>{
  ...
  await expect(page.locator('.start-logo')).toHaveAttribute('src','assets/branding/ever-deeper-logo.png?v=0345');
...
test('settings opens first and keeps audio choices separate from stats',async({page})=>{
  ...
  await expect(page.locator('.patch-note')).toHaveCount(6);
  await expect(page.locator('.patch-note.latest')).toContainText('v0.34.5');
  await expect(page.locator('.patch-note.latest')).toContainText('opens the achievement list');
  await expect(page.locator('.patch-note').nth(4)).toContainText('100 Emberstone');
  await expect(page.locator('.patch-note').nth(4)).toContainText('200 Astralite and 200 Crownstone');
...
test('v0.34.5 exposes the complete production contracts and premium walk renderer',async({page})=>{
  await freshGame(page);
  const snapshot=await page.evaluate(()=>window.__everDeeperTest.snapshot());
  expect(snapshot.build).toEqual({version:'0.34.5',name:'DEEPGLASS PREMIUM'});
  expect(snapshot.assetVersion).toBe('0345');
