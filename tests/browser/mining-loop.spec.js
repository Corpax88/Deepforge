...
test('expanded mine depths use lazy terrain chunks and a following camera',async({page})=>{
  ...
  await page.evaluate(()=>window.__everDeeperTest.enterMine('mossMine'));
  let snapshot=await page.evaluate(()=>window.__everDeeperTest.snapshot());
  expect(snapshot.build).toEqual({version:'0.36.4',name:'NATURAL RECOVERY'});
  expect(snapshot.assetVersion).toBe('03604');
...
test('premium start menu owns continue, new game, achievements, and settings',async({page})=>{
  ...
  await expect(page.locator('.start-logo')).toHaveAttribute('src','assets/branding/ever-deeper-logo.png?v=03604');
...
test('settings opens first and keeps audio choices separate from stats',async({page})=>{
  ...
  await expect(patchNotes).toHaveCount(25);
  await expect(page.locator('.patch-note.latest')).toContainText('v0.36.4');
  await expect(page.locator('.patch-note.latest')).toContainText('flee continuously');
...
test('v0.36.4 exposes the complete production contracts and premium walk renderer',async({page})=>{
  await freshGame(page);
  const snapshot=await page.evaluate(()=>window.__everDeeperTest.snapshot());
  expect(snapshot.build).toEqual({version:'0.36.4',name:'NATURAL RECOVERY'});
  expect(snapshot.assetVersion).toBe('03604');
